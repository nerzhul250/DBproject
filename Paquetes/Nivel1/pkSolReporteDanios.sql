...
BEGIN
SELECT codigo,estado,descripcion,cliente_cedula,producto_codigo,tipoanomalia_id 
into vCodigo,vEstado,vDescripcion,vClienteCedula,vProductoCodigo,vTipoAnomalia_Id 
FROM solreportedanios INNER JOIN solicitud ON solicitud.codigo=solreportedanios.solicitud_codigo WHERE codigo=ivCodigo;
ovconsulta:= vCodigo||','||vEstado||','||vDescripcion||','||vClienteCedula||','||vProductoCodigo||','||vTipoAnomalia_Id;
RETURN ovconsulta;
EXCEPTION
